...
(function node() {
    ...
    apps.build = function node_apps_build(test) {
        ...
        phases = {
                    ...
                    fileList.forEach(function node_apps_build_css_readdir_each(value) {
                        node.fs.readFile(`${projectPath}css${sep + value}`, "utf8", function node_apps_build_css_readdir_each_readfile(erf, fileData) {
                            ...
                            if (filelen < 1) {
                                ...
                                node.fs.writeFile(`${projectPath}css${sep}index.css`, index, "utf8", function node_apps_build_css_readdir_each_readfile_write(erw) {
                                    if (erw !== null) {
            ...
            libraries: function node_apps_build_libraries() {
                ...
                const flag = {
                    ...
                    node.fs.readFile(file, "utf8", function node_apps_build_libraries_modifyFile_read(err, data) {
                        ...
                        if (file.slice(file.length - 3) !== ".js") {
                            node.fs.writeFile(file, data, "utf8", function node_apps_build_libraries_modifyFile_write(err) {
                                if (err !== null) {
                                    apps.errout([err.toString()]);
                                    return;
                                }
                        ...
                        node.fs.readFile(filePath, "utf8", function node_apps_build_libraries_libraryFiles_appendFile_read(errr, filedata) {
                            ...
                            if (filename === "FileSaver.min.js" && filePath.indexOf(filename) === filePath.length - filename.length) {
                                filedata = filedata
                                    ...
                                    .replace(/(\{|\}|;|(\*\/))\s*var\s/g, function node_apps_build_libraries_libraryFiles_appendFile_read_saveAsFix(str) {
                            ...
                            a = a + 1;
                            if (a === filelen) {
                                const inject = function node_apps_build_libraries_appendFile_inject(version) {
                ...
                }, versionGather = function node_apps_build_libraries_versionGather() {
                    node.child(`git log -1 --branches`, function node_apps_build_libraries_versionGather_child(err, stderr) {
                        ...
                        node.fs.readFile(`${projectPath}package.json`, "utf8", function node_apps_build_libraries_versionGather_child_readPackage(errp, data) {
                            ...
                            node.fs.readFile(`${projectPath}node_modules${sep}sparser${sep}package.json`, "utf8", function node_apps_build_libraries_versionGather_child_readPackage_readSparser(errf, frameData) {
                                if (errf !== null) {
                                ...
                                defaults = (function node_apps_build_libraries_modifyFile_read_buildDefault() {
                                    const obj = {};
                                    let a = 0;
                                    do {
            ...
            sparser: function node_apps_build_sparser() {
                ...
                node.fs.stat(`${frame + sep}js${sep}parse.js`, function node_apps_build_sparser(ers) {
                    if (ers !== null) {
                        if (ers.code === "ENOENT") {
                            console.log(`${apps.humantime(false)}Sparser does not appear to be built... building now.`);
                            node.child(`tsc --pretty`, {
                                cwd: frame
                            }, function node_apps_build_sparser_tsc(err, stdout, stderr) {
                                if (err !== null) {
                                    apps.errout([err.toString()]);
                                    return;
                                }
                                ...
                                node.child(`node js${sep}services build`, {
                                    cwd: frame
                                }, function node_apps_build_sparser_tsc_build(erb, stbout, stberr) {
                                    if (erb !== null) {
            ...
            typescript: function node_apps_build_typescript() {
                const flag = {
                    services: false,
                    typescript: false
                }, command = (process.argv[0] === "local")
                    ? "node_modules\\.bin\\tsc --pretty"
                    : "tsc --pretty", ts = function node_apps_build_typescript_ts() {
                    node.child(command, {
                        cwd: projectPath
    ...
    apps.copy = function node_apps_copy(params) {
        ...
        util.dir = function node_apps_copy_dir(item) {
            node
                .fs
                .readdir(item, function node_apps_copy_dir_makedir_readdir(er, files) {
                ...
                apps.makedir(place, function node_apps_copy_dir_makedir() {
                ...
                if (list[index][1] === "directory" || list[index][1] === "link") {
                    ...
                    apps.readFile({
                        ...
                        callback: function node_apps_hash_dirComplete_typehash_callback(data, item) {
                            hashback(data, item, function node_apps_hash_dirComplete_typeHash_callback(hashstring, item) {
    ...
    apps.log = function node_apps_log(output, code, path) {
        ...
        if (code !== "") {
            if (options.output === "" || options.output === undefined) {
                ...
                node.fs.writeFile(out, code, function node_apps_output_writeFile(err) {
                    if (err !== null) {
                        apps.errout([err.toString()]);
                        return;
                    }
            ...
            node
                ...
                if (options.binary_check.test(bstring) === true) {
                    ...
                    node
                        ...
                        .readFile(args.path, {
                        encoding: "utf8"
                    }, function node_apps_readFile_wrapper_stat_file_open_read_readFile(errc, dump) {
    ...
    apps.readMethod = function node_apps_readMethod(diff) {
        ...
        const readmethod = options.read_method, item = (diff === true)
            ? "diff"
            : "source", resolve = function node_apps_readmethod_resolve() {
            node.fs.stat(options[item], function node_apps_readmethod_resolve_stat(err, stat) {
                const screen = function node_apps_readmethod_resolve_stat_screen() {
    ...
    apps.simulation = function node_apps_simulation(callback) {
        const tests = require(`${js}tests${sep}simulations.js`), len = tests.length, cwd = __dirname.replace(/(\/|\\)js$/, ""), increment = function node_apps_simulation_increment(irr) {
