...
(function dom_init() {
    ...
    const id = function dom_id(x) {
        ...
        if (pages === "webtool") {
            ...
            const aceApply = function dom_load_aceApply(nodeName, maxWidth) {
                ...
                const el = event.srcElement || event.target, elId = el.getAttribute("id"), acedisable = function dom_load_acedisable() {
                ...
                const feedradio = function dom_load_feedradio(event) {
                ...
                el.onblur = function dom_load_feedblur() {
            ...
            if (test.audio !== null) {
                ...
                data.audio.play = function dom_load_audio() {
                    const source = test
                        .audio
                        .createBufferSource(), buff = new ArrayBuffer(data.audio.binary.length), bytes = new Uint8Array(buff), bytelen = buff.byteLength;
                    let z = 0;
                    do {
                        bytes[z] = data
                            .audio
    ...
    method.event.colSliderGrab = function dom_event_colSliderGrab(event) {
        let subOffset = 0, withinRange = false, offset = 0, status = "ew", diffLeft, node = event.srcElement || event.target;
